...
app.get("/", async () => {
  return { service: "weather-verify-api", status: "ok" };
});
...
app.post(
  "/api/locations/:id/backfill-forecast",
  { preHandler: requireAuth },
  async (request, reply) => {
    const { id } = IdParamSchema.parse(request.params);
...
app.post(
  "/api/locations/:id/backfill-previous-runs",
  { preHandler: requireAuth },
  async (request, reply) => {
    const { id } = IdParamSchema.parse(request.params);
    ...
    const days = await fetchPreviousRuns(
      location.lat,
      location.lon,
      location.timezone
    );
...
app.get(
  "/api/locations/:id/lead-time-stats",
  { preHandler: requireAuth },
  async (request, reply) => {
    const { id } = IdParamSchema.parse(request.params);
